  ...
  return (
    <div className="page">
      ...
      <section className="panel">
        ...
        <div className="table-wrap"><table className="data-table"><thead><tr><th>Tracking</th><th>Route</th><th>Package</th><th>Status</th><th>Progress</th><th>ETA</th><th>Action</th></tr></thead><tbody>{filteredShipments.map((shipment) => <tr key={shipment.id}><td><Link to={`/shipments/${shipment.trackingNumber}`} style={{ textDecoration: "none", color: "#2563eb", fontWeight: "bold" }}>{shipment.trackingNumber}</Link><div className="subtle" style={{ fontSize: 12 }}>{shipment.id}</div></td><td><strong>{shipment.senderCity}</strong> to <strong>{shipment.receiverCity}</strong><div className="subtle" style={{ fontSize: 12 }}>{shipment.receiverName}</div></td><td>{shipment.packageType}<div className="subtle" style={{ fontSize: 12 }}>{shipment.weight} - {shipment.priority}</div></td><td><span className={`badge ${statusClass(shipment.status)}`}>{shipment.status}</span></td><td style={{ minWidth: 150 }}><div className="progress-track"><div className="progress-fill" style={{ width: `${shipment.progress}%` }} /></div><div className="subtle" style={{ fontSize: 12, marginTop: 4 }}>{shipment.progress}% complete</div></td><td>{shipment.eta}</td><td>{isShipmentAdmin ? <button className="button secondary compact" onClick={() => setSelectedTracking(shipment.trackingNumber)} type="button">Manage</button> : canEditStatus ? <select className="select" onChange={(e) => updateStatus(shipment.trackingNumber, e.target.value, shipment.receiverCity)} value={shipment.status}>{statuses.map((item) => <option key={item}>{item}</option>)}</select> : <Link className="button secondary" to={`/track?num=${shipment.trackingNumber}`}>Track</Link>}</td></tr>)}</tbody></table></div>
